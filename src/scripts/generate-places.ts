...
async function geocode(placeName: string, preferredCountry?: string): Promise<GeocodeResult> {
  ...
  if (!response.ok) {
    throw new Error(`Geocoding failed for ${placeName}: ${response.status} ${response.statusText}`);
  }
...
export async function resolvePlace(
  ...
  const sourceCoordinates = readCoordinates(item.name, item.lat, item.lng);
  ...
  if (!sourceCoordinates && !cachedCoordinates) {
    const searchName = item.nameEn || inferEnglishName(item.name) || item.name;
    geocodeResult = await geocodePlace(searchName, item.country);
    readCoordinates(item.name, geocodeResult.lat, geocodeResult.lng);
  }

  const coordinates = sourceCoordinates || cachedCoordinates || geocodeResult;
  ...
  if (!country) {
    throw new Error(
      `${item.name}: country is required when using explicit coordinates without a cache`
    );
  }
  ...
  return {
    place: {
      name: item.name,
      nameEn: item.nameEn || cachedPlace?.nameEn || inferEnglishName(item.name),
      country,
      ...coordinates,

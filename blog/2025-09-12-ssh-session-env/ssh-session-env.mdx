...
## [@flowblok](https://blog.flowblok.id.au/) 的结论
...
[@flowblok](https://blog.flowblok.id.au/) 在 [《Shell startup scripts》](https://blog.flowblok.id.au/2013-02/shell-startup-scripts.html) 里画的图实在太复杂了，他说他通读了各路资料，不明觉厉啊！
...
让我们来验证一下 [@flowblok](https://blog.flowblok.id.au/) 的结论。
...
- `/etc/environment`（这是 [@flowblok](https://blog.flowblok.id.au/) 没有提到的全局环境文件）
- `/etc/profile`
- `/etc/profile.d/custom.sh`（这是 [@flowblok](https://blog.flowblok.id.au/) 没有提到的自定义脚本）
- `/etc/bash.bashrc`
- `/root/.profile`
- `/root/.bash_login`
- `/root/.bash_profile`
...
结果与 [@flowblok](https://blog.flowblok.id.au/) 写得八九不离十了。

![exp-result-1](assets/exp-result-1.png)

#### 登录交互会话

首先 [@flowblok](https://blog.flowblok.id.au/) 没有提到 `/etc/environment` 文件，这是两种会话都会加载的。
...
这一点在 `man bash` 中的 `INVOCATION` 里已经有所提及（可能 [@flowblok](https://blog.flowblok.id.au/) 也写了而我没仔细读……）。
...
3. 大公司内倾向于造新轮子，从而实现 ClickOps，而非使用 GitOps 范式，使用命令行工具。（当然你可以说这是实践 [Developer Portal](https://www.cncf.io/projects/backstage/) 🤡）
4. 开发平台才能解决 Ansible 解决不了的问题。

## 后记 4

感谢 [cherry-studio](https://github.com/CherryHQ/cherry-studio)。

---

import SponsorIframe from '@site/src/components/SponsorIframe';

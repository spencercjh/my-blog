...
CPU
Limit（Quota）应该是 **容器目录下** 的 `/sys/fs/cgroup/cpu/cpu.cfs_quota_us ÷ /sys/fs/cgroup/cpu/cpu.cfs_period_us`。
...
- 通过 `/proc/${pod-container-pid}/cgroup` 拿到子系统 `(cpu, cpuacct)` 的路径
  ...
  - 通过 `/proc/${pod-container-pid}/mountinfo` 拿到
    ...
    - 在 daemonset pod 中，cgroup 子系统目录需要将 `/sys` 替换成 `/host-sys`（配置卷和挂载点时是这么配的）
      - 计算 `cpu.cfs_quota_us / cpu.cfs_period_us` 的值，使用 `math.Ceil` 向上取整。
...
`command.Process.Pid` 是 `nsexec` 进程的 pid，可以在 `/proc/$pid/task/$pid/children` 找到它的子进程 `chaos_os`。为了确保万无一失，我会对所有的子进程都进行 `choom` 操作。
...
> 移除容器有什么用？看看我的另一篇博客 [《业内首创！服务启动期间注入故障的混沌工程实践》](../2025-09-11-service-startup-exp/startup-exp.mdx)

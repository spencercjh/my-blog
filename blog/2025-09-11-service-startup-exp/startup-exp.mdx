...
在服务启动期间注入网络故障，验证服务启动期间的表现是否符合预期。观察的内容包括但不限于：

1. 依赖是否触发降级逻辑；
2. 依赖是否触发重试逻辑；
3. 服务是否能成功启动；
4. Pod 的 liveness、readiness 和 startup probe 表现是否符合预期。

恢复故障后，观察服务的表现是否符合预期。观察的内容包括但不限于：
...
_下面这张时序图由 GitHub Copilot Agent 使用 GPT-5 模型生成。_
...
关于这个 bug 的修复详见 [《修复 ChaosBlade 遗留多年的 Bug-正确实现移除容器》](../2025-05-09-bcm-engine/3-bugfix.mdx#正确实现移除容器)。
...
_由于太过简单，我没有把这个 bugfix 记录在 [《修复 ChaosBlade 遗留多年的 Bug》](../2025-05-09-bcm-engine/3-bugfix.mdx) 中。_
...
- 如果用户手动恢复故障时容器由于重启回退延迟机制没有启动，这次恢复操作将一定会失败。
  所幸还有演练最大时间耗尽后的自动恢复机制（上层平台写了一个定时任务，chaosblade 自己也有 timeout 机制），
  到那时如果容器还没有启动，网络故障 tc 规则就会残留在 Pod 上，影响正常业务。冗余恢复机制使得我们还没有遇到过这个问题。
- 用户（主要是测试）反映整体使用流程仍然比较繁琐，需要掌握非常多的代码细节。
  这点我无能为力，我认为这就是个白盒测试，应该由写代码的研发来操作，而不是不熟悉代码的测试来操作。
